...
class Game extends Component {
  ...
  componentDidMount() {
    ...
    if (!localStorage.getItem("goodGames"))
      localStorage.setItem("goodGames", 0);
  ...
  keyDown(event) {
    let value = event.key;
    if (ALLOWED_KEYS.includes(value)) this.onKeyClick({ value });
  }
  ...
  setTilesLetters(value) {
    let { letters, curRow } = this.state,
      posChange = letters[curRow].indexOf(" ");
    ...
    if (value === "⏎" || value === "Enter") {
      ...
      const guess = letters[curRow].join("").toLocaleLowerCase(),
        hardMode = localStorage.getItem("hardmode") || false;
      if (hardMode === "false" || curRow === 0) {
        // default
        if (WORD_POOL.includes(guess)) {
          ...
          if (++curRow < MAX_ROWS) this.setActiveTile(curRow, 0, "activeTile");
        ...
        for (let i = 0; i < colors[curRow - 1].length; i++) {
          if (colors[curRow - 1][i] === "rightTile") {
            ...
            if (!chGuess.includes(letters[curRow - 1][i])) {
              ...
              chGuess = this.replaceAt(
                chGuess,
                chGuess.indexOf(letters[curRow - 1][i]),
                "1"
              );
          }
        }
      ...
      // handling any key thats allowed except for enter and backspace
    ...
    if (curRow >= MAX_ROWS) setTimeout(() => this.refresh(), 700); // lose reset
  }

  setActiveTile(curRow, posChange, value) {
    // setting and removing glow on current tile
    let { colors } = this.state;
    if (posChange < 5) colors[curRow][posChange] = value;
    this.setState({ colors: colors });
  }

  setInvalidRowAnimation(value) {
    // setting and removing shake animation
    let { rowAnimations, curRow } = this.state;
    rowAnimations[curRow] = value;
    this.setState({ rowAnimations: rowAnimations });
  }

  setBounceRowAnimation(value) {
    // setting and removing bounce animation
    let { rowAnimations, curRow } = this.state;
    rowAnimations[curRow] = value;
    this.setState({ rowAnimations: rowAnimations });
  }
  ...
  setColorsAccordingToGuess(guess) {
    let { colors, keyColors, curWord, curRow } = this.state,
      rightGuess = curWord,
      win = true;
    for (let i = 0; i < guess.length; i++) {
      if (rightGuess.includes(guess[i])) {
        if (rightGuess[i] === guess[i]) {
          ...
          rightGuess = this.replaceAt(
            rightGuess,
            rightGuess.indexOf(guess[i]),
            "1"
          );
        } else {
    ...
    if (win) {
      ...
      localStorage.setItem(
        "goodGames",
        parseInt(localStorage.getItem("goodGames")) + 1
      );
    }
  }
  ...
  refresh() {
    localStorage.setItem(
      "totalGames",
      parseInt(localStorage.getItem("totalGames")) + 1
    );
    let { curRow } = this.state;
    localStorage.setItem(
      "totalGuesses",
      parseInt(localStorage.getItem("totalGuesses")) + curRow
    );
    let letters = [
        [" ", " ", " ", " ", " "],
        [" ", " ", " ", " ", " "],
  ...
  render() {
    const { letters, colors, keyColors, rowAnimations, curWord, curRow } =
      this.state;
